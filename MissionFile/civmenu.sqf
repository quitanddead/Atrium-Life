...
ctrlSetText [1, format["%1 vous a donnez un ticket de $%2. Voullez-vous le payer?", _cop, (_strafe call ISSE_str_IntToStr)]];
...
if (Antwort == 1) then
	...
	(format ["server globalChat format [localize ""STRS_civmenu_didpayticket"", name %2, %3];[""Ticket"", %2, %3] spawn Isse_AddCrimeLogEntry;if (iscop) then{player groupChat ""Vous avez recu $%4 parce que %2 a payer le ticket."";[""geld"", %4] call INV_AddInvItem;};kopfgeld_%2 = kopfgeld_%2 - %3; if(kopfgeld_%2 < 0)then{kopfgeld_%2 = 0};", _cop, player, (_strafe call ISSE_str_IntToStr), (_copticket call ISSE_str_IntToStr)]) call broadcast;
...
if (_civkopfgeld != 0) then
	...
	(format['if(rolestring == "%1")then{kontostand = kontostand + %2; player groupChat "Le civil avait une prime de $%2! Vous avez recu cette prime!"}', _copobj, _civkopfgeld]) call broadcast;
...
if(stolencash > 0)then
	...
	(format ["server globalchat ""%1 est un voleur de banque et a ete charger de payer $%2!"";", name player, (_INVrmv + _BNKrmv)]) call broadcast;
	_playercaught = player;
	/*
	_copplayernumber = playersNumber west;
	_copbonus = round(stolencash/_copplayernumber);
	if (_copbonus > maxcopbonus) then {_copbonus = maxcopbonus};
	"if (iscop) then {kontostand = (kontostand + _copbonus); player sidechat format[""Vous avez recu votre part de $%1 pour un travail bien fait!"", _copbonus];};" call broadcast;
	*/
	};
...
while {true} do
	...
	hintsilent format["Temp avant de sortir: %1 seconds\n reste a payer: $%2", _timetotake, round(_bounty)];
	...
	if (player Distance prison_logic >= 60) exitWith {_exitart = "ausbruch"};
...
if ((_loopart == "arrest") and (player distance sprisonflag <= 100))  then
...
if (_civkopfgeld != 0) then
	...
	(format['if(rolestring == "%1")then{kontostand = kontostand + %2; player groupChat "Ce civil a une prime de $%2! Vous avez recu la prime!"}', _copobj, _civkopfgeld]) call broadcast;

	};

(format ["server GLOBALCHAT format [localize ""STRS_civmenucheck_arrested_global"", name %1, name %3, %2]; [""PrisonIn"", %1, %3] spawn Isse_AddCrimeLogEntry; kopfgeld_%1 = (%2*10000);%1_wanted = 0;%1_arrest = 1;", player, ((_prisondauer/60) call ISSE_str_IntToStr), _copobj]) call broadcast;

player setpos getmarkerpos "sprisonspawn";
...
while {true} do
	...
	hintsilent format["Temps pour sortir: %1 seconds\nBail reste a payer: $%2", _timetotake, round(_bounty)];

	if (isNull(player))                      exitWith {_exitart = ""};
	if (!(alive player))                     exitWith {_exitart = "tot"};
	if (_counter >= _prisondauer)             exitWith {_exitart = "frei"};
	if (_freigelassen == 0)                   exitWith {_exitart = "freigelassen"};
	if (player Distance prison_logic >= 75) exitWith {_exitart = "ausbruch"};
	if (_bounty <= 0)			  exitwith {_exitart = "freigelassen"};
...
if (_exitart == "ausbruch") then
	...
	(format ["if (player == %1) then {local_arrest = 0; [""local_arrest"", local_arrest] spawn ClientSaveVar;}; %1_arrest = 0; if(!(""S'est evade de prison"" in %1_reason))then{%1_reason = %1_reason + [""escaping from jail""]}; %1_wanted = 1; kopfgeld_%1 = 20000; server GLOBALCHAT format [localize ""STRS_civmenucheck_breakout"", name %1]; [""PrisonEsc"", %1] spawn Isse_AddCrimeLogEntry;", player]) call broadcast;
...
{
(format ['if (rolestring == "%1") then {hint "%2 s'est fait vole recement";};', _aktionsStarter, player]) call broadcast;
};
_geld  = 'geld' call INV_GetItemAmount;
_amounttosteal = (floor(random _geld));
["geld", -(_amounttosteal)] call INV_AddInvItem;

(format ['if (rolestring == "%1") then {["geld", %2] call INV_AddInvItem;};hint "%1 a vole %2 a/de %3";',_aktionsStarter, _amounttosteal, player]) call broadcast;
