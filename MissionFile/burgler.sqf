...
if ((isciv) and (player distance jaild < 5)) then
	{
		...
		player groupchat "La porte de la cellule est deja ouverte.";

		} else {
	player groupChat "Vous essayez de crocheter la porte de votre cellule";
	(format ['server globalChat "Quelqu'un essaie d'ouvrir la porte d'une cellule";']) call broadcast;
	...
	if (round(random 100) <= 30) then
	{
			...
			sleep 8;
			breaking = false;
			player groupChat "Vous avez reussi a ouvrir la porte";
			(format ['server globalChat "Quelqu'un a ouvert avec succès la porte de sa cellule!";']) call broadcast;
			dar_ilpd animate ["CellDoorA",1]; dar_ilpd say "Porte ouverte";
	...
	else
	{
			...
			sleep 8;
			breaking = false;
			player groupChat "Vous n'avez pas reussi a ouvrir votre cellule.";
			(format ["%1 switchmove ""%2"";", player, ""]) call broadcast;
	};
...
{
if ((isciv) and (player distance jaild < 100)) exitwith {player groupChat "Vous etes suffisamment proche de la salle de controle.";};
};
};
