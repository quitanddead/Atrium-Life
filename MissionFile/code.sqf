...
if ((player distance jail2 <= 4) and (_cell == 1)) then
{
	...
	{
	player groupchat "Vous avez besoin d'une arme pour les sortir de prison!";
	};
player groupchat format["Opening Cell %1", _cell];
cell1 setPos [(getPos cell1 select 0),(getPos cell1 select 1),-5];
format['hint "ÉVASION DE PRISON EN COURS!!!";'] call broadcast;
sleep 15;
...
player groupchat "Ouverture de la porte";
sleep 15;
...
player groupchat "Fermeture de la porte dans 15 secondes";
sleep 15;
