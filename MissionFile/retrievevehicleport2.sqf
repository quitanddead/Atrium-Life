...
player removeAction action2227;
reTrieve = player addaction [_rtv,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehWater select 0);',1,true,true,"",'player distance vsave_port2 < 5'];
reTrieve1 = player addaction [_rtv2,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehWater select 1);',1,true,true,"",'player distance vsave_port2 < 5'];
reTrieve2 = player addaction [_rtv3,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehWater select 2);',1,true,true,"",'player distance vsave_port2 < 5'];
reTrieve3 = player addaction [_rtv4,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehWater select 3);',1,true,true,"",'player distance vsave_port2 < 5'];
reTrieve4 = player addaction [_rtv5,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehWater select 4);',1,true,true,"",'player distance vsave_port2 < 5'];
...
_v = format ["vehicle_%1_%2",player,round(time)];
	...
	', player, round(time), INV_CALL_CREATVEHICLE, getpos vsaver_port2, getdir _logic];
...
action2227 = player addaction ["Retrieve Saved Vehicle","retrievevehicleport2.sqf",[],1,false,true,"","player distance vsave_port2 <= 10"];
};