...
if (_outside == 0 and isnil "_nearobj")exitwith {player groupchat "You need to be inside a house to use this"};
{_range = ((getMarkerPos _x) distance (player));
if (_range < _dist) then
{_dist = _range; _marker = _x;};
} foreach Homearray;
if (_marker == "") exitwith {player groupchat "You need to be at an occupied house, its a perk!";};
if ((player distance getmarkerpos _marker <= 10) and (_outside == 1)) exitwith {player groupchat "You need to place this outside the house";};
if ((player distance getmarkerpos _marker >=9) and (_outside == 0)) exitwith {player groupchat "You need to place this inside the house";};
