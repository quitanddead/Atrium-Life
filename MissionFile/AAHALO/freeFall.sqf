...
_float_dirReq = 0;
_float_speedVReq = 0;
//_float_speedHReq = 0; //Direct translation

_float_angleCh = 0;
_float_dirCh = 0;
_float_speedVCh = 0;
//_float_speedHCh = 0; //Direct translation

_obj_ps = "#particlesource" createVehicleLocal getPos _obj_jump;
_obj_ps setParticleParams
[
	[ "\Ca\Data\Cl_water",1,0,1 ],
	"",
	"Billboard",
	1,
	2,
...
while{ _obj_jump getVariable "bool_freeFall" }do
{
	...
	[
		_float_speedH*_float_vX,
		_float_speedH*_float_vY,
		-1*_float_speedV
	];
	_obj_jump setVectorDir
	[
		-1*_float_vX*_float_vZ,
		-1*_float_vY*_float_vZ,
		-1*( ( -1*_float_vX )^2 + _float_vY^2 )
	];
	...
	if( _bool_camera && _obj_jump getVariable "bool_freeFall" )then
	{
		...
		_obj_camera camSetPos
		[
			( _pos_jump select 0 )+ _float_vX*_float_camROSize,
			( _pos_jump select 1 )+ _float_vY*_float_camROSize,
			( _pos_jump select 2 )+_float_camDist
		];
		_obj_camera camSetTarget
		[
			( _pos_jump select 0 )+ _float_vX*_float_camROSize,
			( _pos_jump select 1 )+ 1,
			0
		];
