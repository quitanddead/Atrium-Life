...
if (WahlChief) exitWith {player groupChat "Vous avez vote.";};
...
format ["(INV_ItemTypenArray select 0) SET [2, %1]; (INV_ItemTypenArray select 1) SET [2, %2]; (INV_ItemTypenArray select 2) SET [2, %3]; (INV_ItemTypenArray select 3) SET [2, %4];bank_steuer = %5; hint ""Le maire a changer les taxes!"";", _item, _vcl, _mag, _weap, _bank] call broadcast;
