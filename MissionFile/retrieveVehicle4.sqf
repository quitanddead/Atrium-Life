...
player removeAction action2224;
reTrieve = player addaction [_rtv,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLandCop select 0);',1,true,true,"",'player distance vsave8 < 5'];
reTrieve1 = player addaction [_rtv2,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLandCop select 1);',1,true,true,"",'player distance vsave8 < 5'];
reTrieve2 = player addaction [_rtv3,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLandCop select 2);',1,true,true,"",'player distance vsave8 < 5'];
reTrieve3 = player addaction [_rtv4,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLandCop select 3);',1,true,true,"",'player distance vsave8 < 5'];
reTrieve4 = player addaction [_rtv5,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLandCop select 4);',1,true,true,"",'player distance vsave8 < 5'];
...
_v = format ["vehicle_%1_%2",player,round(time)];
	...
	', player, round(time), INV_CALL_CREATVEHICLE, getpos vsavers8, getdir _logic];
...
action2224 = player addaction ["Retrieve Saved Vehicle","retrievevehicle4.sqf",[],1,false,true,"","player distance vsave8 <= 3"];
};