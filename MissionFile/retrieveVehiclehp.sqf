...
reTrieve = player addaction [_rtv,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLand select 0);',1,true,true,"",'player distance vsaveHP < 5'];
reTrieve1 = player addaction [_rtv2,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLand select 1);',1,true,true,"",'player distance vsaveHP < 5'];
reTrieve2 = player addaction [_rtv3,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehLand select 2);',1,true,true,"",'player distance vsaveHP < 5'];
...
_v = format ["vehicle_%1_%2",player,round(time)];
	...
	', player, round(time), INV_CALL_CREATVEHICLE, getpos ccarspawnthp, getdir _logic];
...
action2225 = player addaction ["Retrieve Saved Vehicle","retrievevehiclehp.sqf",[],1,false,true,"","player distance vsaveHP <= 3"];
};