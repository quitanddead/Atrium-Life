...
//	if (!debug and (player distance deadcamlogic < 300) and alive player and iscop) then {call TeleToMainbaseFunc;};
	...
	{
		if (player distance getmarkerpos "paintballr1" >= 90 and !(_veh isKindOf "Man")) then {player groupchat "Vous entrez dans une zone de paint ball, vous allez être éjecter de votre véhicule si vous continuez, ceci est une zone interdite aux vols!";};
		if ((player distance getmarkerpos "paintballr1" < 88 and player distance getmarkerpos "paintballr1" >= 85) and !(_veh isKindOf "Air")) then
		{
			if (_veh != player) then
				{
				player groupchat "Vous êtes en train de vous faire éjecter de votre véhicule!";
				_veh setvelocity [0,0,0];
				player Action ["eject", _veh];
				player Action ["GET OUT", _veh];
				};
		...
		if ((player distance getmarkerpos "paintballr1" < 120) and (player distance getmarkerpos "paintballr1" >= 50) and (_veh isKindOf "Air")) then
			...
			player groupchat "Vous avez été téléporter hors de la zone de paint ball! Ceci est une zone interdite aux vols";
			};
		if (player distance getmarkerpos "paintballr1" < 120 and player distance getmarkerpos "paintballr1" >= 130) then
		{
		player groupchat "Les armes ne sont pas autorisées dans les terrains de paint ball, tous les armes vous sont maintenant enlevés!";
...
/*
	...
	if (player distance getmarkerpos "pdsub1" < 5 and isciv and !(_veh isKindOf "Air") and !(_veh isKindOf "Ship")) then
		...
		player groupchat "Vous avez été téléporter hors du département du Shériff!";
	...
	if (player distance getmarkerpos "pdsub2" < 15 and isciv and !(_veh isKindOf "Air") and !(_veh isKindOf "Ship")) then
		...
		player groupchat "Vous avez été téléporter hors de l'unité du SWAT!";
